 ...
 * every rate it feeds.
 */
export function countSyllables(word, rule) {
    ...
    for (const ch of cleaned) {
        if (vowels.has(ch)) {
            ...
            if (!inNucleus || !rule.adjacentVowelsMerge)
                count++;
            inNucleus = true;
        }
        else {
...
export function countSyllablesIn(words, rule) {
    let total = 0;
    for (const word of words)
        total += countSyllables(word, rule);
    return total;
}
/**
