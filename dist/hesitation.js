 ...
 */
import { SEARCH_PAUSE_MS } from "./pause.js";
...
const bare = (word) => word.replace(/[.,!?;:…»«"„“”]+$/u, "").replace(/^[«"„“]+/u, "");
export function hesitations(pauses, words, limit = MAX_HESITATIONS) {
    const found = [];
    for (const pause of pauses) {
        const ms = pause.endMs - pause.startMs;
        // Searching, not merely breathing — see SEARCH_PAUSE_MS in pause.ts.
        if (ms < SEARCH_PAUSE_MS)
            continue;
        ...
        if (index < 0)
            continue; // a pause after the last recognised word names nothing
...
//# sourceMappingURL=hesitation.js.map